...
function dr() {
  if (je)
    return z;
  je = 1;
  var p = Te, m = Symbol.for("react.element"), _ = Symbol.for("react.fragment"), b = Object.prototype.hasOwnProperty, h = p.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ReactCurrentOwner, w = { key: !0, ref: !0, __self: !0, __source: !0 };
  function k(i, s, g) {
    var o, a = {}, y = null, E = null;
    g !== void 0 && (y = "" + g), s.key !== void 0 && (y = "" + s.key), s.ref !== void 0 && (E = s.ref);
    for (o in s)
      b.call(s, o) && !w.hasOwnProperty(o) && (a[o] = s[o]);
    if (i && i.defaultProps)
      for (o in s = i.defaultProps, s)
        a[o] === void 0 && (a[o] = s[o]);
    return { $$typeof: m, type: i, key: y, ref: E, props: a, _owner: h.current };
  }
...
function mr() {
  return Pe || (Pe = 1, process.env.NODE_ENV !== "production" && function() {
    var p = Te, m = Symbol.for("react.element"), _ = Symbol.for("react.portal"), b = Symbol.for("react.fragment"), h = Symbol.for("react.strict_mode"), w = Symbol.for("react.profiler"), k = Symbol.for("react.provider"), i = Symbol.for("react.context"), s = Symbol.for("react.forward_ref"), g = Symbol.for("react.suspense"), o = Symbol.for("react.suspense_list"), a = Symbol.for("react.memo"), y = Symbol.for("react.lazy"), E = Symbol.for("react.offscreen"), C = Symbol.iterator, T = "@@iterator";
    function D(e) {
    ...
    function j(e, r, t) {
      {
        var n = l.ReactDebugCurrentFrame, d = n.getStackAddendum();
        d !== "" && (r += "%s", t = t.concat([d]));
    ...
    function Le(e) {
      return !!(typeof e == "string" || typeof e == "function" || e === b || e === w || Ie || e === h || e === g || e === o || Ae || e === E || xe || Fe || Ne || typeof e == "object" && e !== null && (e.$$typeof === y || e.$$typeof === a || e.$$typeof === k || e.$$typeof === i || e.$$typeof === s || // This needs to include all possible module reference object
...
` + c[S].replace(" at new ", " at ");
        ...
        K = !1, X.current = v, Me(), Error.prepareStackTrace = d;
      }
    ...
    function qe(e, r, t, n, d) {
      {
        ...
        for (var u in e)
          if (v(e, u)) {
            ...
            c && !(c instanceof Error) && (V(d), f("%s: type specification of %s `%s` is invalid; the type checker function must return `null` or an `Error` but returned a %s. You may have forgotten to pass an argument to the type checker creator (arrayOf, instanceOf, objectOf, oneOf, oneOfType, and shape all require an argument).", n || "React class", t, u, typeof c), V(null)), c instanceof Error && !(c.message in me) && (me[c.message] = !0, V(d), f("Failed %s type: %s", t, c.message), V(null));
          }
      }
    }
    ...
    function tr(e, r, t, n, d) {
      {
        ...
        t !== void 0 && (he(t), c = "" + t), Qe(r) && (he(r.key), c = "" + r.key), Ke(r) && (O = r.ref, Ze(r, d));
        for (v in r)
        ...
        return rr(e, c, O, d, n, L.current, u);
      }
    }
...
var $e = ne.exports;
const gr = (p, m, _, b) => {
  let h = m / (_ + b), w = Math.floor(p.length / h);
  h > p.length && (h = p.length, w = 1);
  const k = [];
  for (let i = 0; i < h; i++) {
    let s = 0;
    for (let g = 0; g < w && i * w + g < p.length; g++)
      s += p[i * w + g];
    k.push(s / w);
  }
  return k;
  ...
  i && (i.clearRect(0, 0, m.width, m.height), h !== "transparent" && (i.fillStyle = h, i.fillRect(0, 0, m.width, m.height)), p.forEach((s, g) => {
    i.fillStyle = w;
    const o = g * (_ + b), a = k - s / 2, y = _, E = s || 1;
    i.beginPath(), i.roundRect ? (i.roundRect(o, a, y, E, 50), i.fill()) : i.fillRect(o, a, y, E);
  }));
}, Sr = ({
  mediaRecorder: p,
  width: m = "100%",
  ...
  const [a] = Y(() => new AudioContext()), [y, E] = Y(), C = De(null);
  H(() => {
    if (!p.stream)
      return;
    const l = a.createAnalyser();
    E(l), l.fftSize = i, l.minDecibels = g, l.maxDecibels = s, l.smoothingTimeConstant = o, a.createMediaStreamSource(p.stream).connect(l);
  }, [p.stream]), H(() => {
    y && p.state === "recording" && T();
  }, [y, p.state]);
  const T = fr(() => {
    if (!y)
      return;
    const l = new Uint8Array(y == null ? void 0 : y.frequencyBinCount);
    p.state === "recording" ? (y == null || y.getByteFrequencyData(l), D(l), requestAnimationFrame(T)) : p.state === "paused" ? D(l) : p.state === "inactive" && a.state !== "closed" && a.close();
  }, [y, a.state]), D = (l) => {
    if (!C.current)
      return;
    const f = gr(
  ...
  const w = p.getChannelData(0), k = _ / (b + h), i = Math.floor(w.length / k), s = m / 2;
  let g = [], o = 0;
  for (let a = 0; a < k; a++) {
    const y = [];
    let E = 0;
    const C = [];
    let T = 0;
    for (let f = 0; f < i && a * i + f < p.length; f++) {
      const j = w[a * i + f];
      j <= 0 && (y.push(j), E++), j > 0 && (C.push(j), T++);
    }
    const D = y.reduce((f, j) => f + j, 0) / E, l = { max: C.reduce((f, j) => f + j, 0) / T, min: D };
  ...
  const a = (i || 0) / s;
  p.forEach((y, E) => {
    const C = E / p.length, T = a > C;
    o.fillStyle = T && k ? k : w;
    const D = E * (_ + b), l = g + y.min, f = _, j = g + y.max - l;
